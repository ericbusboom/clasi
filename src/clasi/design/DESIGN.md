...
**Storage (`store.py`):** `_subsystem_dirs(root)` enumerates a source root's immediate subdirectories only (hidden dirs and `__pycache__` excluded) — nested directories belong to the subsystem that contains them and never get their own doc. `read_doc_set` walks every configured source root and returns `Artifact` handles (not-necessarily-existing) for: the system doc; each existing declared root's own required root-level `DESIGN.md` overview; and every subsystem's co-located `DESIGN.md`. Root-overview and subsystem entries share one `subsystem_docs` dict, keyed by source path (a declared root path for the overview, a subsystem dir for the rest). `write_design_doc` writes a bare markdown body with no `---` frontmatter block unless a caller explicitly passes `extra_frontmatter`.
