...
- [ ] Session-start hook is created during `clasi init`
- [ ] Hook triggers SE process loading at session start (agent sees the trigger message)
- [ ] Installation is idempotent — running init twice produces the same result, no duplication
- [ ] Existing CLAUDE.md directive is preserved (not removed or modified)
- [ ] Hook format matches the mechanism confirmed in ticket 001
