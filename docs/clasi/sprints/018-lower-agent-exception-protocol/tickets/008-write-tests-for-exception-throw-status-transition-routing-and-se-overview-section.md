---
id: "008"
title: "Write tests for exception throw, status transition, routing, and SE overview section"
status: todo
use-cases:
  - SUC-001
  - SUC-002
...
- [ ] `test_update_ticket_status_accepts_exception`: `update_ticket_status`
  with `status="exception"` succeeds and writes `exception` to frontmatter.
- [ ] `test_update_ticket_status_rejects_unknown_still`: unknown status still
  raises `ValueError` after adding `exception`.
- [ ] `test_throw_ticket_exception_writes_frontmatter`: calling
  `throw_ticket_exception` writes all five fields of the `exception:` block
  to the ticket frontmatter.
- [ ] `test_throw_ticket_exception_sets_status_exception`: after calling
  the tool, ticket `status` field is `"exception"`.
- [ ] `test_throw_ticket_exception_returns_thrown_at`: return JSON contains
  `thrown_at` as an ISO-8601 string.
- [ ] `test_throw_ticket_exception_invalid_thrown_by`: `ValueError` / JSON
  error on invalid `thrown_by`.
- [ ] `test_throw_ticket_exception_invalid_surface`: `ValueError` / JSON
  error on invalid `surface`.
- [ ] `test_throw_ticket_exception_unknown_path`: error on non-existent path.

### Unit tests — `tests/unit/test_sprint.py`
- [ ] `test_ticket_counts_includes_exception_bucket`: `ticket_counts()`
  returns `{"open": 0, "in_progress": 0, "done": 0, "exception": 1}` when
  one ticket has `status: exception`.

### Unit tests — `tests/unit/test_ticket.py`
- [ ] `test_exception_payload_absent`: `Ticket.exception_payload` returns
  `None` when no `exception` key in frontmatter.
- [ ] `test_exception_payload_present`: returns dict with all fields when
...
- [ ] `test_throw_and_list`: create a sprint + ticket, call
  ...
  returns that ticket.
- [ ] `test_exception_ticket_blocks_pre_close`: assert
  `review_sprint_pre_close` returns an error (not success) when a ticket
  with `status: exception` exists in the sprint.

### SE overview update
- [ ] `clasi/se-overview-template.md` gains an "Exception protocol" section
...
### Regression
- [ ] `uv run pytest` passes with no regressions across all existing tests.
