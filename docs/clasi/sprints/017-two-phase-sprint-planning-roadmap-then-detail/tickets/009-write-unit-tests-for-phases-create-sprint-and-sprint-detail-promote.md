...
- [ ] `test_roadmap_is_first_phase`: `PHASES[0] == "roadmap"`.
- [ ] `test_advance_from_roadmap`: `StateDB.advance_phase()` from `roadmap` yields `planning-docs`.
- [ ] `test_create_sprint_writes_only_sprint_md`: after `Project.create_sprint(title)`, only `sprint.md` exists in the sprint dir; `usecases.md`, `architecture-update.md`, `tickets/` are absent.
- [ ] `test_create_sprint_status_roadmap`: `sprint.md` frontmatter has `status: roadmap`.
- [ ] `test_detail_promote_scaffolds_artifacts`: `Sprint.detail_promote()` on a roadmap sprint writes `usecases.md`, `architecture-update.md`, creates `tickets/` and `tickets/done/`, and phase becomes `planning-docs`.
- [ ] `test_detail_promote_rejects_non_roadmap`: calling `detail_promote()` on a sprint already in `planning-docs` raises `ValueError`.
- [ ] `test_detail_promote_idempotent_guard`: manually creating `usecases.md` then calling `detail_promote()` raises `ValueError`.
