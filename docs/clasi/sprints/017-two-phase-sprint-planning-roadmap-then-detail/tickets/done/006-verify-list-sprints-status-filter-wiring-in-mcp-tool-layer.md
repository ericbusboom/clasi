...
- [x] `list_sprints` MCP tool accepts an optional `status` parameter.
- [x] Calling `list_sprints(status="roadmap")` returns only sprints whose `sprint.md` has `status: roadmap`.
- [x] Calling `list_sprints()` (no argument) returns all sprints regardless of status.
