...
- [x] `_deep_diff` is implemented in `clasr/merge.py` as a private module-level function.
- [x] `_reverse_diff` is implemented in `clasr/merge.py` as a private module-level function.
- [x] Neither function mutates its inputs.
- [x] `_deep_diff({}, {"a": 1})` returns `{"a": 1}`.
- [x] `_deep_diff({"a": 1}, {"a": 1})` returns `{}` (equal values: no contribution).
- [x] `_deep_diff({"a": 1}, {"a": 2})` returns `{"a": 2}` (scalar conflict).
- [x] `_deep_diff({"x": {"p": 1}}, {"x": {"p": 1, "q": 2}})` returns `{"x": {"q": 2}}`.
- [x] `_reverse_diff({"a": 1, "b": 2}, {"a": 1})` returns `{"b": 2}`.
- [x] `_reverse_diff({"x": {"p": 1, "q": 2}}, {"x": {"q": 2}})` returns `{"x": {"p": 1}}`.
- [x] `_reverse_diff({"a": 1}, {"b": 2})` returns `{"a": 1}` (missing key: skipped).
- [x] `_reverse_diff({"x": {"p": 1}}, {"x": {"p": 1}})` returns `{}` (entire sub-dict removed).
- [x] No new imports from `clasi` or other `clasr` modules.
- [x] `uv run pytest tests/clasr/test_merge.py` passes (no regressions).
