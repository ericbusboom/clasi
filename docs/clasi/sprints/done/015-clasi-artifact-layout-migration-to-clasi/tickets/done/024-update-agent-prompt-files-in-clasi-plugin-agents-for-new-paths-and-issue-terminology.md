...
- [x] All agent prompts in `clasi/plugin/agents/` use "issue" terminology
- [x] All agent prompts reference `.clasi/` not `docs/clasi/`
- [x] `status: open` used for unstarted tickets in prompts (not `status: todo`)
- [x] `/issue` skill referenced (not `/todo`)
- [x] `grep -rn "docs/clasi" clasi/plugin/` returns zero hits
