...
- [x] `clasi init` creates `.clasi/issues/` (pending pool) — no in-progress or done subdirs
- [x] `clasi init` creates `.clasi/log/`, `.clasi/sprints/`, `.clasi/architecture/`,
  `.clasi/reflections/`
- [x] No `docs/clasi/` directories are created
- [x] `tests/unit/test_init_command.py` assertions updated for new layout
