...
- [x] `clasr/registry.py` exists with `INTEGRATION_REGISTRY`, `get()`, `detect()`.
- [x] `get("claude")` returns a `ClaudeIntegration` instance.
- [x] `get("unknown")` raises `KeyError`.
- [x] `detect(target)` returns correct integrations based on `detect_files` presence.
- [x] `test_integration_contract.py` is updated to parametrize over `INTEGRATION_REGISTRY.values()`.
- [x] Contract test passes for Claude, Codex, and Copilot (three parametrized runs).
