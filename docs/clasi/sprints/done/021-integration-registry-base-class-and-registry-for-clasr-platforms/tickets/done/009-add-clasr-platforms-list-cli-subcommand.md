...
- [x] `clasr platforms list` prints registered platform IDs, one per line, sorted, to stdout.
- [x] Exit code is 0.
- [x] `clasr platforms` (no subcommand) prints help and exits 0.
- [x] `clasr platforms list` output includes `cursor` after ticket 008 is done.
- [x] `uv run pytest tests/clasr/test_cli.py` passes (updated for new subcommand).
