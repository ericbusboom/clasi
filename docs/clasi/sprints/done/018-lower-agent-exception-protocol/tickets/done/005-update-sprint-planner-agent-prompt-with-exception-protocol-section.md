...
- [x] `clasi/plugin/agents/sprint-planner/agent.md` contains a clearly
  delimited "Exception Protocol" section.
- [x] Section states the threshold (same as programmer: structural wall, not
  hard work).
- [x] Section instructs the sprint-planner to call `throw_ticket_exception`
  with `thrown_by="sprint-planner"` when a ticket exists to carry the payload.
- [x] Section explains the pre-ticket case: surface exception payload in
  return text using the same five-field schema, clearly marked.
- [x] Section defines `surface` classification (same rules as programmer).
- [x] Section instructs clean exit — do not leave partial artifacts.
- [x] No existing content in `agent.md` is removed or materially altered.
- [x] No tests to write (documentation change only).
