...
- [ ] `PHASES` remains a module-level list named `PHASES` (no callers are changed).
- [ ] The existing list is renamed to `_PHASES_FALLBACK` and kept at module level.
- [ ] When `CLASI_SCHEMA_PHASES` env var is `"1"`, `PHASES` is derived by loading `se-process/schema.yaml` via `ArtifactGraph.phases()`.
- [ ] When `CLASI_SCHEMA_PHASES` env var is absent or any other value, `PHASES = _PHASES_FALLBACK` (unchanged behavior).
- [ ] A startup check asserts that the schema-derived list equals `_PHASES_FALLBACK` when the flag is active; logs a WARNING and falls back if they differ.
- [ ] The schema path used is resolved relative to the `clasi/schemas/` package directory (using `importlib.resources` or `Path(__file__).parent`).
- [ ] All existing `state_db_class.py` tests pass unchanged.
- [ ] New test: `CLASI_SCHEMA_PHASES=1` yields a `PHASES` list identical to `_PHASES_FALLBACK`.
