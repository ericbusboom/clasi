...
- [ ] `loader.load(path: str | Path) -> WorkflowSchema` parses the YAML at `path` using PyYAML `safe_load`.
- [ ] Pydantic validation runs on the parsed dict; unknown fields raise `SchemaError` wrapping the Pydantic error.
- [ ] Duplicate `id` values across `artifacts` raise `SchemaError` naming the duplicated ID.
- [ ] Any `requires` entry that references an unknown artifact `id` raises `SchemaError` naming the missing ID.
- [ ] A cycle in the `requires` graph raises `SchemaError` identifying the cycle (Kahn's algorithm or DFS).
- [ ] Any `gate.kind` not in the registry `{"stakeholder-review", "review", "per-ticket"}` raises `SchemaError` naming the unknown kind.
- [ ] A valid schema returns a `WorkflowSchema` instance with `artifacts` in topological order.
- [ ] `loader.py` has zero imports from `clasi.*` (only stdlib, pyyaml, pydantic, and the models from `clasi.schemas`).
