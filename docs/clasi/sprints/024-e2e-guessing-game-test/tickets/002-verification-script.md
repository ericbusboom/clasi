...
- [ ] `tests/e2e/verify.py` exists and accepts a project directory argument
- [ ] Checks application functionality (starts, menu, each game)
- [ ] Checks 4 sprint directories exist in `done/` with correct status
- [ ] Checks all tickets are in `done/` directories with done status
- [ ] Checks dispatch logs exist and contain content
- [ ] Runs the project's test suite and checks exit code
- [ ] Prints a pass/fail summary with counts
- [ ] Exit code reflects overall result (0 = all pass)
- [ ] Importable as a module (`verify(project_dir)` function)
