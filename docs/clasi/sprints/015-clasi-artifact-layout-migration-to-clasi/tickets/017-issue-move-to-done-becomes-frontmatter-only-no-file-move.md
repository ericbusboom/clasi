...
- [ ] `Issue.move_to_done` only writes frontmatter: `status: done`, `tickets:` list
- [ ] No file is moved or renamed in `move_to_done`
- [ ] The issue file stays at `<sprint>/issues/<filename>` after `move_to_done`
- [ ] `artifact_tools.py` validates the issue is in the correct sprint's `issues/` dir
- [ ] Tests verify: file location unchanged after done; `status: done` in frontmatter
