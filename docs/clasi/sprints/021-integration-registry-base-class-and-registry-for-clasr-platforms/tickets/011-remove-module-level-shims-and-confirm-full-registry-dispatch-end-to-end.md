...
status: todo
use-cases: [SUC-003, SUC-004, SUC-005, SUC-007]
...
- [ ] `claude.py`, `codex.py`, `copilot.py` have no module-level `install()` or `uninstall()` free functions.
- [ ] All three modules export only `ClaudeIntegration`, `CodexIntegration`, `CopilotIntegration` classes (plus private helpers).
- [ ] All test files that previously imported the free functions are updated to use class API.
- [ ] `uv run pytest` green — no import errors, no broken references.
- [ ] `clasr install --claude --codex --copilot --source ... --provider ...` completes end-to-end without errors.
