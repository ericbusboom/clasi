...
- [ ] Existing test `test_merge_json_files_basic` updated: assert `contributed` is a `dict`
      containing `{"beta": 2, "gamma": 3}`.
- [ ] Existing test `test_merge_json_files_returns_contributed_keys` updated: assert
      `contributed` is `{"mcpServers": {"my-server": {}}}` (full value, not `["mcpServers"]`).
- [ ] New test: `test_deep_diff_absent_key` — key absent from base: full value returned.
- [ ] New test: `test_deep_diff_equal_values_excluded` — equal values not included in diff.
- [ ] New test: `test_deep_diff_scalar_conflict` — overlay scalar wins; included in diff.
- [ ] New test: `test_deep_diff_nested_partial` — only new nested leaf in diff, not full sub-dict.
- [ ] New test: `test_reverse_diff_removes_key` — removes a contributed key from current.
- [ ] New test: `test_reverse_diff_nested` — removes a nested contributed leaf.
- [ ] New test: `test_reverse_diff_missing_key_skipped` — missing key in current is silently skipped.
- [ ] New test: `test_reverse_diff_entire_sub_dict_removed` — empty after reverse yields key removed.
- [ ] New test: `test_merge_json_files_round_trip` — merge then reverse reproduces base
      (for non-overlapping incoming).
- [ ] New test: `test_reverse_diff_public_export` — `reverse_diff(current, diff)` is callable
      and produces correct result.
- [ ] `uv run pytest tests/clasr/test_merge.py` passes — all new and updated tests green.
