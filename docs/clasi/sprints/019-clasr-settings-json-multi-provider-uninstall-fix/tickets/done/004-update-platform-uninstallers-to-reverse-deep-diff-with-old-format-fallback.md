...
- [x] `claude.py` uninstall: new-format manifest (`"contributed"` present) uses `merge.reverse_diff`.
- [x] `claude.py` uninstall: old-format manifest (no `"contributed"`) falls back to top-level key pop + WARNING.
- [x] `codex.py` uninstall: same symmetric change.
- [x] `copilot.py` uninstall: same symmetric change.
- [x] Uninstalling provider2 (contributed `model` + `permissions`) leaves provider1's `model` and
      `permissions` values intact in `settings.json`.
- [x] Uninstalling provider1 after provider2 is gone deletes `settings.json` (file becomes empty).
- [x] Old-format uninstall emits WARNING containing the path and provider name.
- [x] Old-format uninstall still completes successfully (not an error).
- [x] `uv run pytest tests/clasr/` passes.
