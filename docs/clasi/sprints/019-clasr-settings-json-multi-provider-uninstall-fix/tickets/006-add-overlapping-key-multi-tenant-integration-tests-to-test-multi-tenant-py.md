...
- [ ] `TestJsonMergeUninstallOverlapping` class is added with these test methods:
- [ ] `test_c2_1_uninstall_b_leaves_a_model_intact`: both providers set `model`; uninstall B
      leaves A's `model` value in `settings.json`.
- [ ] `test_c2_2_uninstall_b_leaves_a_permissions_intact`: both providers set `permissions.allow`
      (different lists); uninstall B leaves A's `permissions` key intact.
- [ ] `test_c2_3_uninstall_both_deletes_file`: after uninstalling B then A, `settings.json` is deleted.
- [ ] `test_c2_4_old_format_manifest_fallback_emits_warning`: manually craft old-format manifest
      entry (no `"contributed"` field); call `claude_platform.uninstall()`; confirm WARNING
      on stderr and uninstall completes without error.
- [ ] `test_provider_b_manifest_records_json_merged` in `TestJsonMergeInstall` is updated to also
      assert `"contributed"` is present and is a `dict` in provider_b's manifest entry.
- [ ] All existing `TestJsonMergeUninstall` tests still pass.
- [ ] `uv run pytest tests/clasr/test_multi_tenant.py` passes.
