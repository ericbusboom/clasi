...
- [x] `_log_hook_event`/`_exit_hook` accept an optional `decisions:
      list[str]` parameter; a call with none produces an unchanged log
      line (verify against a snapshot of today's line format).
- [x] `handle_role_guard` accumulates decision tokens across its checks
      and passes them through on every exit path.
- [x] `handle_mcp_guard` accumulates decision tokens across its checks
      and passes them through on every exit path.
- [x] Every `hooks.log` line for a decision that has tokens carries those
      tokens; every guard decision line (allow or deny) is covered by at
      least one token.
- [x] Every denial (`exit_code == 2`) writes the full hook payload to
      `.clasi/log/denied/<ts>-<hook>.json`.
- [x] `.clasi/log/denied/` gets its own `.gitignore` via the existing
      `_ensure_log_gitignore` mechanism.
- [x] `handle_plan_to_issue` and `handle_codex_plan_to_issue` are routed
      through `_exit_hook` (not raw `sys.exit`), so `plan-to-issue`/
      `codex-plan-to-issue` events appear in `hooks.log` for the first
      time.
- [x] A guard-internal exception (in `handle_role_guard` or
